    ...
    def process_video(
        ...
        **kwargs,
        ...
        results = self.pipeline.process_video(
            video_path, output_dir=output_dir, **kwargs)
        ...
        if hasattr(self.pipeline, "config"):
            config = self.pipeline.config.__dict__ if hasattr(
                self.pipeline.config, "__dict__") else {}
...
def quick_evaluate(
    ...
    if output_dir:
        output_dir = Path(output_dir)
        reporter.save_json(result, output_dir /
                           f"{video_name}_evaluation.json")
        reporter.save_markdown(result, output_dir /
                               f"{video_name}_evaluation.md")
